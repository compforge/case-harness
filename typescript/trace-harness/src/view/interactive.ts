...
.pane{flex:1;min-width:0;overflow:auto;padding:16px}.row{white-space:nowrap;cursor:pointer;font-size:12px;padding:2px 8px;border-left:3px solid transparent;display:flex;align-items:baseline;gap:6px}.row.agent-row{border-left-color:var(--node-color,#9ca3af)}
...
if(perspective==='agent'){row.classList.add('agent-row');row.style.setProperty('--node-color',KCOLOR[n.kind]||'#9ca3af');row.style.minHeight=rowHeight+'px';row.style.alignItems='center';}
