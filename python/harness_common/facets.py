...
not a harness domain concept. eval and perf both classify their cases by facets and order
report groups by them, so the model lives here once. Each harness supplies its own base
registry (if any) and projects its cases' facet dims through `validate_dimensions` /
`order_key`; the per-case facet *values* live on each harness's own case object.
