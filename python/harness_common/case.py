"""Canonical case — the harness-neutral INPUT contract (see ``spec/case-schema.yaml``),
the mirror of ``common.verdict`` on the output side.
...
NOT own (left to each harness): the *content* — ``input`` semantics and the per-face
``judge.<face>`` criteria fields are harness-autonomous; each harness projects a ``Case``
into its own internal model. ``id`` is the primary key joining input → output
(``case.id`` == ``verdict.case_id``) and across runs/faces.
"""
...
from harness_common import FACES
...
class Case:
    ...
    ``input`` is a protocol-agnostic request description interpreted by each harness's
    ...
    all optional, inner fields harness-owned.
    """
...
class CaseSet:
    ...
    One canonical type all three faces read; there is no per-harness Case class."""
...
def load_caseset(path: str | Path) -> CaseSet:
    """Parse a canonical case file (``spec/case-schema.yaml`` shape) → ``CaseSet``. Does
...
def case_from_raw(c: dict) -> Case:
    ...
    ``from_raw`` — exposed so a harness that owns its own set container (eval's ``EvalSet``
...
def case_to_raw(case: Case) -> dict:
    ...
    ``CaseVerdict.to_dict`` on the output side. Omit-empty so a hand-written case stays terse
...
def validate(cs: CaseSet) -> None:
    ...
    - every ``case.facets`` key is a declared facet, values in-vocab (`common.facets`);
    - every ``requires`` name resolves to a declared source;
    - every ``judge`` key is a known face (`FACES`); inner criteria are harness-owned.
    """
