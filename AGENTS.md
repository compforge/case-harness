...
- **Trajectory 专门化**：trajectory_harness 使用 `Trajectory / Measurements / Detector / Verifier` 作为领域概念；Measurements 从 Trajectory 确定性派生，二者共同输入 Detector 与 Verifier。后两者均可面向 `cost / effect`，也均可声明 `hard / soft` 规则，不暴露 Evaluator 概念。
