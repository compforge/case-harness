...
func (s *Suite) Finish() (report.RunVerdict, string, error) {
	...
	if !pathSegmentPattern.MatchString(s.scope) {
		return verdict, "", fmt.Errorf("e2e suite scope %q is not a safe path segment", s.scope)
	}
	if !pathSegmentPattern.MatchString(s.runID) {
		return verdict, "", fmt.Errorf("e2e suite run id %q is not a safe path segment", s.runID)
	}
